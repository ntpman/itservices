@extends('layouts.adminlte')

@section('page_name')
    | Brand Models Edit
@endsection

@section('content')
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-6">
                <div class="card card-primary card-outline">
                    <div class="card-header">
                        <h3 class="card-title"><i class="far fa-edit"></i> แก้ไขข้อมูลรุ่นผลิตภัณฑ์</h3>
                    </div>
                    <!-- form start -->
                    {!! Form::open(['action' => ['Basics\BrandModelController@update', $editBrandModel->id], 'method'=>'PUT']) !!}
                    <div class="card-body">
                        <div class="form-group">
                            {{ Form::label('title','ยี่ห้อผลิตภัณฑ์') }}                            
                            {{ Form::select('brandId', $allBrand, $editBrandModel->brand_id, ['class' => 'form-control']) }}
                        </div>
                        <div class="form-group">
                            {{ Form::label('title','ชื่อรุ่นผลิตภัณฑ์')}}
                            <a href="#" data-toggle="modal" data-target="#edit-brandModelName">
                                <i class="far fa-edit"></i> Edit
                            </a>
							{{ Form::text('', $editBrandModel->brand_model_name, ['class' => 'form-control', 'readonly']) }}
                        </div>
                        <div class="form-group">
                            {{ Form::label('title', 'สถานะการใช้งานข้อมูล') }}
                            {{ Form::select('brandModelStatus', [
                                    'A' => 'Active',
                                    'D' => 'Disable',
                                ], $editBrandModel->brand_model_status, ['class'=>'form-control']) 
                            }}
                        </div>
                        <a href="/basics/brandmodel" class="btn btn-secondary">ย้อนกลับ</a>
                        <input type="hidden" name="edit" value="1">
                        {{ Form::hidden('_method','PUT') }}
                        {{ Form::submit('บันทึก', ['class'=>'btn btn-primary']) }}
                    </div>
                    {!! Form::close() !!}
                    <!-- end start -->
                </div>
                <!-- /.card -->
            </div>
            <!-- /.col-md-6 -->
        </div>
        <!-- /.row -->
    </div><!-- /.container-fluid -->
@endsection

@section('modal')

    @include('basics.brandmodel.modal.edit-brandmodel-name')

@endsection

@section('scripts')
    
@endsection